...
describe RaffleService do 
    ...
    describe "#call"do 
        context 'when has more then two members' do
            ...
            it "all members get/take someone" do
                result_members = @results.map { |r.first| } #map{x => z, a => b, d => e} = first {x,a,d}
                expect(result_members.sort).to eq(@campaign.members.sort)
            end

            it "all member are in results as a friend" do
                result_friends = @results.map { |r.last| }
                expect(result_friends.sort).to eq(@campaign.members.sort)
            end

            it "a member dont't ger yourself" do
                @results.each do |r|
        ...
        context "when don't has more then two members" do
            before(:each) do
                ...
                campaing.reload
                @resposnse = ReflleService.new(@campaign).call
            end
